...
| Hinglish Model | Trelis/whisper-hinglish-preview | 1.55B params, Whisper large-v3 based, specifically trained for Hinglish code-switching, best accuracy |
...
- Apple Silicon Mac — M1 / M2 / M3 / M4 / M5 (strongly recommended)
- Minimum 16GB RAM (24GB recommended for running Whisper model + LLM simultaneously)
...
| Hotkey | Action |
|---|---|
| Hold `Fn` key | Hold-to-Talk: Starts recording when pressed down, stops recording and transcribes when released. |
| Double Tap `Fn` key | Toggle Continuous Mode: Locks recording state on. Press `Fn` once more to stop and transcribe. |
...
**Goal:** Add a persistent visual state indicator so the user knows if the background process is listening, transcribing, or done.
**Status:** 🟡 IN PROGRESS

Tasks:
- [ ] Create `hs.canvas` floating pill UI in `init.lua`
- [ ] Implement 3 states: 🔴 Listening, 🟡 Processing, 🟢 Done
- [ ] Make the UI click-through (ignore mouse events) and always-on-top
- [ ] Position it at the bottom-center of the screen
- [ ] Sync UI states with the `Fn` key hold and double-tap logic
...
| Phase 2 | whisper.cpp Build | ✅ Complete | 100% |
| Phase 3 | Hinglish Model Download | ✅ Complete | 100% |
| Phase 4 | Core Dictation Engine | ✅ Complete (Option B) | 100% |
| Phase 5 | Hammerspoon Hotkeys | 🟡 In Progress | 0% |
| Phase 5.5 | Visual Indicator (Floating Pill UI) | 🟡 In Progress | 0% |
| Phase 6 | Ollama Text Cleanup | ⬜ Not Started | 0% |
| Phase 7 | GitHub Repository Push | ✅ Complete | 100% |
| Phase 8 | LinkedIn Post | ⬜ Not Started | 0% |
| **TOTAL** | | 🟡 **In Progress** | **~55%** |
...
**→ START PHASE 5: Hammerspoon Hotkey Integration**

### Next commands:
1. Create `hammerspoon/init.lua` config file
2. Build `hs.eventtap` logic for the `Fn` key
3. Implement Hold-to-Talk (Hold `Fn`)
4. Implement Toggle Continuous Mode (Double Tap `Fn`)

When Phase 5 is done:
- Update Phase 5 status to ✅ Complete
- Move to Phase 6 — Ollama Text Cleanup
...
- **Step 1: Option B (indic-transliteration + Simple Regex)** — DEFAULT FOR DAILY DICTATION (0.001s, zero extra RAM)
- **Step 2: Option A (Ollama LLM)** — OPTIONAL FOR LONG FORM BAKWAAS (Capitalization, formatting, filler-word removal)

### Post v1.0 Ideas
- [ ] **UI Overhaul (v2.0):** Replace `hs.canvas` with an HTML Overlay (`hs.webview`) for highly polished, fluid Wispr-like animations. (Currently overkill for v1, but top priority for future polish).
...
### 2026-08-23 — MODEL UPGRADE DECISION + PHASE 3 COMPLETE
- ✅ Phase 3 Complete — Oriserve Apex model downloaded (1.62GB)
- ✅ bakwaas.py core engine built and tested
- ✅ CORRECTIONS dictionary added for proper nouns
- 🔄 Model upgrade decision: Switching to Trelis/whisper-hinglish-preview for better Hinglish accuracy
- 📊 RAM confirmed: 24GB — all models fit comfortably
- 🎯 Work Plan: Option A selected — Whisper model runs when needed, LLM pauses temporarily
- 🟡 Phase 4 in progress — switching model + final testing

### 2026-08-23 — PHASE 1 & 2 COMPLETE + GITHUB LIVE
- ✅ Phase 1 Complete — All dependencies installed (torch, transformers, sounddevice, pyaudio, numpy, etc.)
- ✅ Phase 2 Complete — whisper.cpp built successfully with Apple Silicon Metal acceleration
- ✅ GitHub repo live — [https://github.com/Tauqeer7Khan/Bakwaas](https://github.com/Tauqeer7Khan/Bakwaas)
- ✅ PROJECT OWNER details updated — Tauqeer Khan
- ✅ SYSTEM REQUIREMENTS updated — M5 added
- 🟡 Phase 3 starting next — Hinglish Model Download
